...
If MainKeyHandle Then
   ...
   If rtn = ERROR_SUCCESS Then 'if the key could be opened then
      ...
      Else                        'otherwise, if the value couldnt be retreived
         GetDWORDValue = "Not Found"  'return Error to the user
...
If MainKeyHandle Then
   ...
   If rtn = ERROR_SUCCESS Then 'if the key could be opened
      ...
      If rtn = ERROR_SUCCESS Then 'if the value could be retreived then
         rtn = RegCloseKey(hKey)  'close the key
         '
         'delete the null caracter in the string
         tmpbuffer = sBuffer
         For I = 1 To lBufferSize
            If Asc(Mid(tmpbuffer, I, 1)) Then
                X = X + Mid(tmpbuffer, I, 1)
            End If
         Next I
         
         GetBinaryValue = X 'return the value to the user
      Else                        'otherwise, if the value couldnt be retreived
         GetBinaryValue = "Not Found" 'return Error to the user
...
'usage GetStringValue("HKEY_LOCAL_MACHINE", "String Value")
